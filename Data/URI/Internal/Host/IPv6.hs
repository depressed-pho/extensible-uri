...
bIPv6Addr ∷ (GV.Vector v Word16, Eq (v Word16)) ⇒ v Word16 → Builder
...
isKnownToBeIPv4Embedded ∷ ∀v. (GV.Vector v Word16, Eq (v Word16))
                        ⇒ v Word16
                        → Bool
...
bIPv4EmbeddedV6Addr ∷ ∀v. GV.Vector v Word16 ⇒ v Word16 → Builder
...
bIPv4EmbeddedV6Addr v6 = bOrdinaryIPv6Addr (GV.take 6 v6) ⊕
    ...
    where
      -- NOTE: This can be implemented using stream operations but I
      -- don't think it's worth it.
      repack ∷ GV.Vector v' Word8 ⇒ v Word16 → v' Word8
      {-# INLINEABLE repack #-}
      repack v = GV.fromList
...
bOrdinaryIPv6Addr ∷ GV.Vector v Word16 ⇒ v Word16 → Builder
{-# INLINEABLE bOrdinaryIPv6Addr #-}
bOrdinaryIPv6Addr _ = error "FIXME"

bZoneID ∷ Maybe (CI ByteString) → Builder
{-# INLINEABLE bZoneID #-}
bZoneID Nothing  = (∅)
